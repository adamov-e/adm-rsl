...
type
  ...
  public
    ...
    procedure Refresh;
    function  WriteToPrinter(Smes:String; Slen:Word):LongWord;
    procedure StartDocToPrinter();
    procedure EndDocToPrinter();
    procedure StartPageToPrinter();
...
procedure TPrinter.StartDocToPrinter();
var
  DocInfo: TDocInfo1;
begin
  CheckPrinting(False);
  ...
  SetAbortProc(DC, AbortProc);
  StartDocPrinter(FPrinterHandle, 1, @DocInfo);
  StartPageToPrinter();
end;
...
Procedure TPrinter.EndDocToPrinter();
begin
  CheckPrinting(True);
  EndPagePrinter(FPrinterHandle);
  if not Aborted then EndDocPrinter(FPrinterHandle);
  FPrinting := False;
  FAborted := False;
  FPageNumber := 0;
end;

procedure TPrinter.StartPageToPrinter();
begin
  CheckPrinting(True);
  EndPagePrinter(FPrinterHandle);
...
function  TPrinter.WriteToPrinter(Smes:String; Slen:Word):LongWord;
var N:LongWord;
begin
  WritePrinter(FPrinterHandle, PChar(Smes), sLen, N);
  Result:=N;
end;
