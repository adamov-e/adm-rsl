...
var
  ...
  TaeClipboardEntrys:TMethodEntrys;
...
begin
  ...
  {Методы}
  i:=0;
  TaeClipboardEntrys[i]:=RSL_METH('Get', @TaeClipboard.RSLGetBuffer);

  Inc(i);
  TaeClipboardEntrys[i]:=RSL_METH('Set', @TaeClipboard.RSLSetBuffer);

  Inc(i);
  TaeClipboardEntrys[i]:=RSL_METH('Clear', @TaeClipboard.RSLClearBuffer);

  {Свойства}
  Inc(i);
  TaeClipboardEntrys[i]:=RSL_PROP_METH('Remote', @TaeClipboard.get_Remote, @TaeClipboard.put_Remote);

  Inc(i);
  TaeClipboardEntrys[i]:=RSL_INIT(@TaeClipboard.RSLInit);

  Inc(i);
  TaeClipboardEntrys[i]:=RSL_CTRL(@TaeClipboard.RSLCreate);

  Inc(i);
  TaeClipboardEntrys[i]:=RSL_DSTR(@TaeClipboard.RSLDone);

  Inc(i);
  TaeClipboardEntrys[i]:=RSL_TBL_END();

  PaeClipboardTable^.clsName:='TaeClipboard';
  PaeClipboardTable^.userSize:=SizeOf(TaeClipboard);
  PaeClipboardTable^.Entrys:=@TaeClipboardEntrys[0];
  PaeClipboardTable^.canInherit:=TRUE;
  PaeClipboardTable^.canCvtToIDisp:=TRUE;
  PaeClipboardTable^.Ver:=1;
  PaeClipboardTable^.autoPtr:=NIL;

end;

Initialization
  New(PaeClipboardTable);
  MakeTaeClipboard();
Finalization
  Dispose(PaeClipboardTable);
End.
